...
def plot_clustering(z_run, labels, engine='matplotlib', download=True, folder_name=None):
    """
    ...
    def plot_clustering_matplotlib(z_run, labels, folder_name, download):
        labels = labels[:z_run.shape[0]] # because of weird batch_size
        ...
        if download:
            path = Path(folder_name + "/tsne.png")
            if not path.parent.exists():
                path.parent.mkdir(parents=True)
            plt.savefig(path)
        else:
    ...
    plot_clustering_matplotlib(z_run, labels, folder_name, download)
