...
class VRAE(BaseEstimator, nn.Module):
    ...
    def __init__(self, sequence_length, number_of_features,
                 dataset="protocol", num_tokens=257, pad_id=256,
                 hidden_size=90, hidden_layer_depth=1, latent_length=20,
                 batch_size=32, learning_rate=0.0005, block='LSTM',
                 n_epochs=100, dropout_rate=0.2, optimizer='Adam', loss='MSELoss', cuda=True,
                 print_every=100, clip=True, max_grad_norm=5, dload='.', device="cuda:0"):
        ...
        if dataset == "protocol":
            self.embedding = torch.nn.Embedding(num_tokens, number_of_features, padding_idx=pad_id)
    ...
    def forward(self, x):
        ...
        if self.dataset == "protocol":
            self.x_embed = self.embedding(x.to(torch.long))
        elif self.dataset == "ECG":
            self.x_embed = x

        cell_output = self.encoder(self.x_embed)
    ...
    def compute_loss(self, X):
        ...
        x = X.to(self.device)
        x_decoded, _ = self(x)
        loss, recon_loss, kl_loss = self._rec(x_decoded, self.x_embed, self.loss_fn)
    ...
    def _train(self, train_loader):
        ...
        for t, X in enumerate(train_loader):
            ...
            if self.dataset == "protocol":
                ...
                raise ValueError("Dataset must be chosen in protocol/ECG")
    ...
    def fit(
        ...
        for i in range(self.n_epochs):
            ...
            if save:
                self.save(model_prefix, i, save_model_every)
    ...
    def save(self, model_prefix, i, save_model_every):
        if model_prefix is None:
            raise ValueError("model path is None")
        if (i+1) % save_model_every == 0:
            model_path = Path(model_prefix + "_epoch_{}.pt".format(i + 1))
